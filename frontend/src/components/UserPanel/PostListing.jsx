...
const PostListing = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axiosInstance.post("/listing/post", formData, {
        headers: {
  ...
  return (
    <>
      <div className="fixed z-99 absolute left-5 top-5">
        <button
          className="mb-2 flex items-center justify-center  shadow-lg border-1 border-gray-100 px-2 py-1 rounded-lg text-gray-600 hover:text-gray-600 hover:bg-gray-100 gap-2 transition duration-2 ease-in "
          onClick={() => navigate("/")}
        >
          <FaArrowLeft size={12} />
          Home
